...
ESLint v9 への対応を進めていると、まだ、いくつか Flat Config に対応していない Plugin や Config に遭遇することがあります。その際に、Legacy Config を Flat Config でどのようにすれば利用できるかなどを調べるのに便利だったため、まとめました。
...
- `configs` 以外は大体一緒
  - ※  Flat Config では、`processor` のキーに拡張子 (`.md` など)が利用できないという点もある
- `configs` フィールドがそれぞれの Config の形式になっている
- Flat Config Plugin では、`configs` フィールドを省略し、別モジュールに分けてもよい
